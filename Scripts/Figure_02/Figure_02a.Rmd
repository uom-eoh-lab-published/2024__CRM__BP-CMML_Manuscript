...
https://api.data.mendeley.com/active-data-entities/datasets/confidentials/{dataset_id}/{version}/files/{file_id}/contents
```


```{r}
url <- "https://downloads.data.mendeley.com/downloads/66eea6ac-7ae8-4e0a-a7ba-0cf551d8dc92?token=456765df-cfd2-4984-b442-b998af62c95e&filename=Data.zip"

destfile <- "/Users/kgurashi/GitHub/2024__CRM__BP-CMML_Manuscript/Data/RNASeq/Resources/Gene_Metadata/Data.zip"
...
```{r}
STAR_Salmon_output_path <- paste(data_folder, '/RNASeq/Counts/Raw_counts/Individual', sep = '')
...
saveRDS(TXI.Genes, file = paste(data_folder, '/RNASeq/Counts/Raw_counts/Tximport_object.rds', sep = ''))
```

## - Exporting gene-level counts and TPMs

```{r}
dir.create(paste(data_folder, '/RNASeq/Counts/Raw_counts/Grouped', sep = ''), showWarnings = F)

dir.create(paste(data_folder, '/RNASeq/Counts/Processed_counts', sep = ''), showWarnings = F)
dir.create(paste(data_folder, '/RNASeq/Counts/Processed_counts/Grouped', sep = ''), showWarnings = F)

write.csv(x = TXI.Genes$counts, file = paste(data_folder, '/RNASeq/Counts/Raw_counts/Grouped/Tximport_raw_countmat.csv', sep = ''), row.names = TRUE, quote = FALSE)

write.csv(x = TXI.Genes$abundance, file =  paste(data_folder, '/RNASeq/Counts/Processed_counts/Grouped/Tximport_TPM_mat.csv', sep = ''), row.names = TRUE, quote = FALSE)
```
...
```{r echo=FALSE, fig.align='center', fig.cap='Figure S2. PCA plot showing samples’ coordinates for top three principal components ranked by variance explained and identified by PCA analysis of VST normalised count data. Samples are coloured by disease state/WHO classification as indicated', fig.height=8.66142, fig.pos='H', fig.width=13.7795}
...
```{r echo=FALSE, fig.align='center', fig.cap='Volcano plot of differentially expressed genes comparing BP- CMML to healthy age-matched control HSPCs.', fig.height=8.66142, fig.pos='H', fig.width=13.7795}
