...
```{r}
Expression_matrix <- read.csv(paste(data_folder, '/InHouse/RNASeq/Counts/Processed_counts/Grouped/DESeq2_vst_normalised_countmat.csv', sep = ''), row.names = 'X')
...
Samples_metadata <- read.csv(paste(data_folder, '/InHouse/RNASeq/Metadata/InHouse/Samples_Metadata.csv', sep = ''), row.names = 'Patient.ID')
```
...
```{r}
dir.create(paste(data_folder, '/InHouse/RNASeq/Genie3', sep = ''), showWarnings = F)
```

The code below should be ran on a HPC or highly performing computer.

```{r}
genie3_weights <- paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Weights.rds', sep = '')

if (file.exists(genie3_weights)) {
  
  weightMat <- readRDS(paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Weights.rds', sep = ''))
...
saveRDS(object = weightMat, 
        file = paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Weights.rds', sep = ''))

}
```

```{r}
linkList <- paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Linklist.rds', sep = '')

if (file.exists(linkList)) {

linkList <- readRDS(paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Linklist.rds', sep = ''))
...
saveRDS(linkList, 
        paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Linklist.rds', sep = ''))
...
```{r}
dir.create(paste(data_folder, '/InHouse/RNASeq/WGCNA', sep = ''), showWarnings = F)
...
```{r}
#png(filename = '/Users/kristiangurashi/Dropbox (The University of Manchester)/Projects data/Project__Christie_BP_CMML_Stratification_and_Drug_Prediction/04__Christie_BP_CMML_BulkRNAseq/03__GRN_Analyses/WGCNA/Results/Hierarchical_plot_with_unassigned_genes.png', width = 4.99*3.5, height = 4.88*1.5, units = 'cm', res = 300)
...
### - Plotting OTU networks

```{r eval=FALSE, include=FALSE}
disssim_signed <- 1 - distM_signed_filtered
diag(disssim_signed) = NA

#png(filename = '/Users/kristiangurashi/Dropbox (The University of Manchester)/Projects data/Project__Christie_BP_CMML_Stratification_and_Drug_Prediction/04__Christie_BP_CMML_BulkRNAseq/03__GRN_Analyses/WGCNA/Results/TOM_plot_preMEmerging.png', width = 4.99*2, height = 4.88*2, units = 'cm', res = 300)
...
### - Quantitate Module-Trait correlations
...
### - Plotting Samples MEs expression

```{r eval=FALSE, include=FALSE}
breaks_val <- seq(from = -0.4, to = 0.4, length.out = 13)
col_heatmap_guide <- colorRamp2(breaks = breaks_val,colors = rev(brew_mix))

ME_expmat <- t(MEs)[,rownames(Samples_metadata)]

ME_expmat <- ME_expmat[c('ME6','ME5','ME4','ME3','ME2','ME1'),]

Heatmap_ME_exp <- ComplexHeatmap::Heatmap(ME_expmat, 
                             name = 'Pearson\nCor.',
                            col = col_heatmap_guide,
                            ...
                            cluster_rows = F,
                            border = TRUE,
                            column_split = Samples_metadata$Condition,
                            column_gap = unit(c(2.5), "mm"),
                            cluster_row_slices = F, 
                            cluster_column_slices = F, 
                            row_gap = unit(0.05, 'cm'),
                            show_row_dend = FALSE, 
...
#png(filename = '/Users/kristiangurashi/Dropbox (The University of Manchester)/Projects data/Project__Christie_BP_CMML_Stratification_and_Drug_Prediction/04__Christie_BP_CMML_BulkRNAseq/03__GRN_Analyses/WGCNA/Results/ME_Exp_heatmap_wLegend.png', width = 6.8*2.5, height = 4 *2, units = 'cm', res = 300)

draw(Heatmap_ME_exp, heatmap_legend_side = "bottom", 
     annotation_legend_side = "bottom")
...
write.csv(x = genes_to_module_df, file = paste(data_folder, '/InHouse/RNASeq/WGCNA/ME_Gene_Assignment.csv', sep = ''),
          row.names = T)
...
write.csv(Module_Membership_df, paste(data_folder, '/InHouse/RNASeq/WGCNA/ME_Gene_Membership.csv', sep = ''), 
          quote = F, row.names = T)
```

# 6) Association of WGCNA with pathways and TFs

## - Loading WGCNA MEs

```{r}
MEs <- read.csv(paste(data_folder, '/InHouse/RNASeq/WGCNA/ME_Gene_Membership.csv', sep = ''), row.names = 'X')

MEs_assigned_genes <- read.csv(paste(data_folder, '/InHouse/RNASeq/WGCNA/ME_Gene_Assignment.csv', sep = ''))
...
write.list(Top_30percent_Module_genes_list, 
           file = paste(data_folder, '/InHouse/RNASeq/WGCNA/WGCNA_Modules_Top30_percent_genes.csv', sep = ''))
```
...
```{r}
Genie3_weights <- readRDS(paste(data_folder, '/InHouse/RNASeq/Genie3/Genie3_Weights.rds', sep = ''))
```
...
```{r}
tf_weights <- paste(data_folder, '/InHouse/RNASeq/WGCNA/WGCNA_Modules_Genie3_TF_regulator_per_module.csv', sep = '')
...
if (file.exists(tf_weights)) {
  ...
  write.csv(x = tf_weights, file = paste(data_folder, '/InHouse/RNASeq/WGCNA/WGCNA_Modules_Genie3_TF_regulator_per_module.csv', sep = ''), quote = F, row.names = T)
...
write.list(Enriched[[module]], file = paste(data_folder, 
                                            sprintf('/InHouse/RNASeq/WGCNA/%s_enrichR_pathways.csv', module), 
                                            sep = ''))
