...
```{r}
Expression_matrix <- read.csv(paste(data_folder, '/RNASeq/Counts/Processed_counts/Grouped/DGE_DESeq2_vst_normalised_countmat.csv', sep = ''), row.names = 'X')
...
```{r}
Human_TFs <- read.csv(paste(data_folder, '/RNASeq/Resources/Gene_Metadata/Lambert2018_TFs.txt', sep = ''), sep = '\t', header = F)
...
```{r}
dir.create(paste(data_folder, '/RNASeq/Genie3', sep = ''), showWarnings = F)
```

The code below should be ran on a HPC or highly performing computer.

```{r}
genie3_weights <- paste(data_folder, '/RNASeq/Genie3/Genie3_Weights.rds', sep = '')

if (file.exists(genie3_weights)) {
  
  weightMat <- readRDS(paste(data_folder, '/RNASeq/Genie3/Genie3_Weights.rds', sep = ''))
...
saveRDS(object = weightMat, 
        file = paste(data_folder, '/RNASeq/Genie3/Genie3_Weights.rds', sep = ''))
...
saveRDS(linkList, 
        paste(data_folder, '/RNASeq/Genie3/Genie3_Linklist.rds', sep = ''))
```
...
```{r}
dir.create(paste(data_folder, '/RNASeq/WGCNA', sep = ''), showWarnings = F)
...
NewModuleColors[grep(TRUE,NewModuleColors == 'green')] <- 'firebrick2'
NewModuleColors[grep(TRUE,NewModuleColors == 'red')] <- 'darkgoldenrod1'
...
```{r}
MEs <- NewMEs$newMEs
colnames(MEs) <- c('ME1','ME2','ME3','ME6','ME5','ME4')
MEs <- MEs[,c('ME1','ME2','ME3','ME6','ME5','ME4')]
```
...
Heatmap_ME_exp <- Heatmap(ME_expmat, 
                             name = 'Pearson\nCor.',
                            col = col_heatmap_guide,
...
geneModuleMembership <- as.data.frame(cor(WGCNA_Input_ExpMat_filtered, MEs, use = "p"))
...
# 4. Association of WGCNA with pathways and TFs
...
## B) Sorting modules for highly correlated representative genes
...
if (websiteLive) {
    Enriched[['M1']] <- enrichr(Module_genes_list$M1, dbs_selected)
    Enriched[['M2']] <- enrichr(Module_genes_list$M2, dbs_selected)
    Enriched[['M3']] <- enrichr(Module_genes_list$M3, dbs_selected)
    Enriched[['M4']] <- enrichr(Module_genes_list$M4, dbs_selected)
    Enriched[['M5']] <- enrichr(Module_genes_list$M5, dbs_selected)
    Enriched[['M6']] <- enrichr(Module_genes_list$M6, dbs_selected)
}
...
write.list(Enriched[[module]], file = sprintf('/Users/kristiangurashi/Dropbox (The University of Manchester)/Projects data/Project__Christie_BP_CMML_Stratification_and_Drug_Prediction/04__Christie_BP_CMML_BulkRNAseq/03__GRN_Analyses/WGCNA/Results/%s_enrichR_pathways.csv', module))
