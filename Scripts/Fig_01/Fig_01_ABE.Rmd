...
writeLines(capture.output(sessionInfo()), file.path(script_folder, 'Fig_01_SessionInfo.txt'))
...
```{r echo=FALSE, fig.align='center', fig.cap='Figure 1A', fig.height=8.66142, fig.pos='H', fig.width=13.7795}
...
Comparison of whole mutations distributions across cohorts. Significant differences identified were highlighted in the plots illustrated via GraphPad (see under ***GraphPad/Fig_01/Panels_BE.pzfx***)..
...
```{r echo=FALSE, fig.align='center', fig.cap='Additional figures. Contributions of Manchester (CHRISTIE) and Taipei (NTUH) cohorts to pulled BP-CMML/MDS/MPN groups', fig.height=4, fig.pos='H', fig.width=4}
...
for (n in 1:length(datasets)) {
  ...
  plot <- ggplot(data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=category)) +
    geom_rect() +
    geom_text(x=0.5, aes(y=labelPosition, label=label, color=category), size=6) +
